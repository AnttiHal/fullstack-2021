...
test('renders content', () => {
  ...
  const element = screen.getByText('title that should be visible', {
    exact: false,
  })
...
test('<BlogForm /> call is made with right props', async () => {
  ...
  const titleField = screen.getByPlaceholderText('write title', {
    exact: false,
  })
  const authorField = screen.getByPlaceholderText('write author', {
    exact: false,
  })
